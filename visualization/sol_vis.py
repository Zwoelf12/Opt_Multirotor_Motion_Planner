...
def visualize_solution(robot,
	...
	axs.plot(timeVec_scvx[1:], int_err_small_dt_scvx)
	axs.set_title("integration error SCVX (explicit euler with 10 times smaller step size)")
	axs.set_ylabel("int err small dt []")
	if robot.type == "dI":
		leg = ["p_x", "p_y", "p_z", "v_x", "v_y", "v_z"]
	# axs[2].legend(leg, loc="lower center", bbox_to_anchor=(0.5, -0.3))
	else:
		leg = ["p_x", "p_y", "p_z", "v_x", "v_y", "v_z",
			   "q_1", "q_2", "q_3", "q_4", "w_x", "w_y",
			   "w_z"]
	...
	if robot.type == "fM":
		...
		ax4.legend(["w_x", "w_y", "w_z"])
		ax4.set_xlabel("time [s]")
		ax4.set_ylabel("rotational velocities [rad/s]")
		ax4.set_title("rotational velocities")
